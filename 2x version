...
from random import*
window = Tk()
c = Canvas(window, height=400, width=600, bg='black')
...
tlay = c.create_text(300, 220, text='[L]PLAY', fill='black', font=('', 15))
title = c.create_text(300, 90, text='ASTEROID', fill='white', font=('', 30))
Quit = c.create_rectangle(0, 0, 80, 40, fill='white', outline='white')
tuit = c.create_text(300, 270, text='[Q]QUIT', fill='black', font=('', 15))
...
def move(d):
    if d == "Up" and not c.coords(player)[1] <= 0:
        ...
        c.move(player, -10, 0)
    elif d == "Right" and not c.coords(player)[2] >= 600:
        c.move(player, 10, 0)
    elif d == "Down" and not c.coords(player)[3] >= 400:
        c.move(player, 0, 10)
def ccreate():
    return put_block(randint(0, 59), 'pink')
def reload():
...
def move_block(num):
    c.move(num, 0, 10)
def create():
    return put_block(randint(0, 590), choice(['green', 'red', 'yellow', 'blue']))
def stringer(List):
    s = ''
    for i in List:
        s = s + str(i)
